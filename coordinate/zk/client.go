...
import (
	...
	"github.com/eahydra/go-zookeeper/zk"
)
...
func (c *Client) shell(fn func(conn *zk.Conn) error) error {
	if err := fn(c.conn); err != nil {
		...
		if time.Since(c.dialAt) > time.Second {
			if err := c.reset(); err != nil {
				log.Debugln("zkclient reset connection failed. err:", err)
			}
		}
		return err
	}
...
func (c *Client) mkdir(conn *zk.Conn, path string) error {
	...
	if err != nil && errNtEqual(err, zk.ErrNodeExists) {
		return err
	}
	return nil
}
...
func (c *Client) Delete(path string) error {
	...
	err := c.shell(func(conn *zk.Conn) error {
		...
		if err != nil && errNtEqual(err, zk.ErrNoNode) {
			return err
		}
		return nil
	})
...
func (c *Client) Read(path string, must bool) ([]byte, error) {
	...
	err := c.shell(func(conn *zk.Conn) error {
		...
		if err != nil {
			if errEqual(err, zk.ErrNoNode) && !must {
				return nil
			}
			return err
		}
...
func errNtEqual(err1, err2 error) bool {
