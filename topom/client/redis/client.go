...
func (c *Client) EnableKeySpace() error {
	_, err := redigo.String(c.Do("INFO", "keyspace", "1"))
	if err != nil {
		return errors.Trace(err)
	}
	return nil
...
func (c *Client) Info() (string, map[string]string, error) {
...
func (c *Client) InfoKeySpace() (string, map[int]string, error) {
...
func (c *Client) InfoFull() (string, map[string]string, error) {
	text, info, err := c.Info()
	if err != nil {
		return "", nil, errors.Trace(err)
	}
...
func (p *Pool) EnableKeySpace(addr string) error {
	c, err := p.GetClient(addr)
	if err != nil {
		return err
	}
	defer p.PutClient(c)
	return c.EnableKeySpace()
}

func (p *Pool) Info(addr string) (string, map[string]string, error) {
	c, err := p.GetClient(addr)
	if err != nil {
		return "", nil, err
	}
...
func (p *Pool) InfoFull(addr string) (string, map[string]string, error) {
	c, err := p.GetClient(addr)
	if err != nil {
		return "", nil, err
	}
...
func (s *InfoCache) getSlow(addr string) (string, map[string]string, error) {
